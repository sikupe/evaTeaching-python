...
OUT_DIR = 'de'  # output directory for logs
...
class DifferentialMutation:
    def __init__(self, num_ind, F, decrease):
        self.num_ind = num_ind
        self.F = F
        self.decrease = decrease

    def next_generation(self):
        self.F *= self.decrease

    def __call__(self, ind, pop):
        random_ind_indices = np.random.choice(np.arange(len(pop)), self.num_ind)
        np_pop = np.array(pop)
        random_inds= np_pop[random_ind_indices]
        start_ind = random_inds[0]
        compare_inds = random_inds[1:]

        end_ind = np.mean(compare_inds, axis=0)

        return ind + self.F * (end_ind - start_ind)
...
def mutation(pop, mutate, mut_prob):
    return [mutate(p, pop) if random.random() < mut_prob else p[:] for p in pop]
...
def evolutionary_algorithm(pop, max_gen, fitness, operators, mate_sel, mutate_ind, cross_ind, *, map_fn=map, log=None):
    evals = 0
...
if __name__ == '__main__':
    ...
    for num_mut_ind in [2, 3, 4]:
        for mut_decr in [1., MUT_DECR]:
            for cross_decr in [1., CROSS_DECR]:
                for fit_gen, fit_name in zip(fit_generators, fit_names):
                    name = f'{EXP_ID}-{num_mut_ind}-{mut_decr}-{cross_decr}.{fit_name}'
                    fit = fit_gen(DIMENSION)
                    d_cross = DifferentialCrossover(CR, cross_decr)
                    d_mut = DifferentialMutation(num_mut_ind, F, mut_decr)
                    ...
                    for run in range(REPEATS):
                        ...
                        pop = evolutionary_algorithm(pop, MAX_GEN, fit, [xover, mut], tournament_selection, d_mut,
                                                     d_cross,
                                                     map_fn=map,
                                                     log=log)
                ...
                plt.yscale('log')
                plt.legend()
                plt.savefig(f'{OUT_DIR}/{num_mut_ind}-{mut_decr}-{cross_decr}-stats-combined.png')
                plt.clf()
