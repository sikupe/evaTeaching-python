...
OUT_DIR = 'baldwin'  # output directory for logs
...
def fitness_baldwin(ind, fitness_fun, iterations: int):
    optim_ind = ind
    for _ in range(iterations):
        grad = gradient(optim_ind, fitness_fun)
        norm_grad = grad / np.linalg.norm(grad)
        optim_ind = ind - MUT_STEP * norm_grad
    return fitness_fun(optim_ind)
...
if __name__ == '__main__':
    ...
    for fit_gen, fit_name in zip(fit_generators, fit_names):
        ...
        d_mut = DifferentialMutation(2, F, 1)
        ...
        fit = functools.partial(fitness_baldwin, fitness_fun=fit, iterations=10)
